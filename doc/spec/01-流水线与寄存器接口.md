...
（Q6 复算 238 是按 `fm=3`；Q24 定 `fm=4` 后为 239。`spec/00` §4.8 的 μop 位量按 239 修正。）
...
| `fdirty` | 1 | 该条目在 `S` 期分配过 PRF 目的号（回滚归还判据；Q8：原缺此位，`spec/00` §7.3 已承诺） |
...
（Q2：`commit_t.exc_vld` 改 1 bit、异常包改挂 `exc_pkt`(177)；C10 复核后新增 `is_sret` 组别位 → **49 bit**；评审记录处置列所写 50 与成员表不符，以成员表为准，差异已登记且不追改评审记录。）
...
深度 8 的依据：CSR 写必须按序且提交宽度为 4，8 项覆盖两拍；溢满时 `R` 级反压（与 BP2 同口径）。队列位量随 `spec/10` 定稿回填 `spec/00` §4.8。）
...
|---|---|---|---|---|---|
| BP1 | `rob_near_full`（`ROB_ENTRIES - free < ROB_NEAR_FULL`，Q31：全篇只引用 `spec/00` 的绝对水位，原 `ROB_NEAR_FULL_THRESH` 不存在且属双口径） | `S` 级停止分发 | 组合门控，不冲刷 | ROB 有 ≥ `DECODE_WIDTH` 空闲 | 不变式 **I1：`ROB 可提交条目数 > 0`（至少一个槽能被撤销）**，直接决定不死锁 |
...
**映射历史栈（Q17 定死方案）**：每分发一拍为每个写目的寄存器的 μop 压入 `{arch, old_paddr}`；逐条回滚只读该栈，
**不依赖 ROB 条目**——否则“冲刷当拍就清 ROB 尾指针并释放投机项”会让回滚无数据可用。ROB 释放与回滚就此解耦，
同时解开与 `rob_near_full` 同拍重算的矛盾。栈条目定死 = `{arch[4:0], old_paddr[5:0], br_mark, rob_id[7:0]}` = **20 bit**（非分支条目 `br_mark=0`、`rob_id=0`，不参与比较）；深度 = 参数 `HIST_DEPTH`（**待 `spec/04` 定**）。
**双写者仲裁（C8）**：free-list 的两个写者——提交回收与回滚归还——**作用集合不相交**（提交即不可撤销；回滚只针对未提交条目），同一物理号不存在“既被回滚又被提交回收”的路径，故无需同拍仲裁；若实现层面出现同拍同号两次归还，属实现缺陷，由 §7 A13 断言兜底。**栈深度推导归 `spec/04`**（ISS-003 未决即污染冻结面，一并记入）。
...
| A6 | **持有唯一性**：`$onehot0(ckpt_held_vec)`，其中 `ckpt_held_vec[i] = \|{在途条目 : needs_ckpt==1 && ckpt_id==i}`（B2：原式用已废除的 `2'b11` 哨兵值且 `ckpt_free_list` 形态未定义）；释放判据 = `needs_ckpt && ROB 头提交` | §5.2 `R` |
...
| A13 | free-list 归还唯一性（C8）：`$onehot0(commit_fl_ret_vec \| rollback_fl_ret_vec)`（同拍同一物理号不得被两路归还） | §5.3 |
...
| 自检项 | 复核手段（可复现命令） | 本轮结果 |
|---|---|---|
| 27 个 struct 合计 = 成员逐项相加 | `python script/width_check.py` | **可机器核对 28 / 不符 0 / 未能核实 0**（留档 `doc/05` R-002） |
...
| 待同步清单 | 逐条跟踪 | `spec/10`（Q27/Q29/Q30 + `csrq` 命中/`wdata_sel` 编码 + `is_sret` 生产点）、`spec/06`（Q33）、`spec/04`（Q16/Q17/Q18 + `HIST_DEPTH`/`ROLLBACK_MAX`）、`spec/00` §9（`PERF_W` 与项数，C4）、`iss/README.md` §3（Q19① 映射表，C3：**以本篇 §7 为准**）与 §4（签名表扩 `WRITE_MEM`，C2）、`doc/02`（Q19③/Q29）、tb 超时（Q37） |

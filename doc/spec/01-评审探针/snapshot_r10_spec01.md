...
由本篇 §7 的 SVA 与 `spec/11`（该篇待建） 的 lint 规则共同保证（C6：原写“`spec/01` §8”系引用错位）（综合前用 `$onehot0`/`inside` 断言，
不用 SV 2023 新语法）。
...
| `pc` | 40 | |
| `rob_id` | 8 | |
| `rd_paddr` | 6 | 目的 PRF 号（**H-01 补**：`iq_wr_t.rd_paddr`（§3.8）在 `S` 级存进 IQ 后，发射时必须随本接口送出——`wb_t.paddr`（§3.10）、`x_complete_t.paddr`（§3.20）、`mem_req_t.rd_paddr`（§3.13，再经 §3.16 `dcache_rsp_t.rd_paddr` 透传）全依赖该值；`iq_entry_t` 不存此域，无法靠 IQ 回读，缺它则写回侧无目的号载体） |
...
合计（逐成员相加）= 1+4+10+64+64+44+40+8+6+2+1+1+1+1+12+2+6+5+1+40+8+2+5 = **328 bit** × 6
（Q14：原 293 且缺预测反馈 4 域，导致前端反馈与 checkpoint 恢复在接口链上断裂，已补齐；N5：再补冲刷键 `ftq_id` 后为 322；**H-01：补 `rd_paddr` 后为 328**，同步 `spec/00` §4.8 级间 μop 寄存）
...
**指令边界 walk 规则（P5 内部，BLK-08③ 定案；N-04 修正）**：窗口槽号 0..15，槽地址 = `win_base_pc` + 2×槽号；**块首槽 = 块起始 `pc[4:1]`**（`win_base_pc[1:0]` 恒 00，无定位信息，不作定位键）。①从块首槽起逐槽判定：槽半字低 2 bit ≠`11` → 16 bit，下一起始 = 当前+1；= `11` → 32 bit，占当前槽与当前+1 槽，下一起始 = 当前+2（**起始可落任意槽号**，由指令字节对齐决定，无"只落偶数槽"约束）；②32 bit 指令起始槽 =**15** 时跨窗口尾（后半不在窗内）→ **恢复机制待定（ISS-039）**：R0 2026-09-23 选定「推迟到 `spec/03` 出**块布局限制**细则后收口」，候选机制（窗尾寄存器拼接 / FTQ `cont` 位 / 34B 双窗）见 `doc/00` ISS-039 行，**G1 前必须闭合**；在本情形闭合前，本条**无可行实现**——窗口按 `win_base_pc` 32B 对齐，"随下一窗口重取"会把后半个字当作新指令起始（误译），原地重取则死循环，而本规则禁跨 32B 边界拼接；③`err_pos` 命中槽处截断：其后槽不再产出起始，错误经 `err_pos` 展开为 §3.3 按 lane 的 `err`/`err_code`。
...
**csrq 的四条硬规则（第 6 轮 BLK-01/02/03 补齐；第 10 轮 H-05 补第 4 条）**：
...
4. **两个写口的"命中失败"语义（H-05）**：`S` 级写 `src_val`、`E` 级写 `old_val` 两写口，均**只在 `rob_id` 命中且该条目仍有效时写入**；未命中（含条目已被规则 1 冲刷截断、或写入到达时该项已提交出队）一律**静默丢弃**，不得改写任何其他条目——若实现成"无条件写固定槽"，迟到写回会污染邻条目；反之，本队列能容忍迟到写回，正是靠"入队序=程序序 + 截断作废 + 写口按 `rob_id` 命中"这三条合力。
...
**快照字段的位宽与载体（H-04 补齐）**：`FL_ck[5:0]`——与 free-list 分配指针**同宽**（`PRF_ENTRIES=64` ⇒ 6 bit，`spec/00` §4.3）；`SP_ck`——宽度 = `log2(HIST_DEPTH)`，`HIST_DEPTH` 现为待定（`spec/00` §4.3 占位，"待定"），**故 `SP_ck` 位宽随该参数定值后代入，本成员标记待定**；快照存储量 = `CHECKPOINT_NUM × (RAT 整表 + FL_ck + SP_ck) = 4 × (192 + 6 + 待定)` bit，归 `spec/04` 容量表与 `spec/00` §4.8（X4/ISS-003：`HIST_DEPTH` 未定即污染冻结面）。
...
| 自检项 | 复核手段（可复现命令） | 本轮结果 |
|---|---|---|
| 28 个 struct 合计 = 成员逐项相加，且**标题位宽数与实算一致** | `python script/width_check.py`（v0.3 起含 N16①②③：标题核对、NO-CLAIM 不再计为可核对、转录基准行单列） | **可机器核对 28 / 不符 0 / 未能核实 0 / 基准行 2**（留档 `doc/05` R-005；第 9 轮复核复跑同值，R-009 / `doc/spec/01-评审探针/machine_r9_width.txt`） |
...
| 表格与台账机械核销 | `python script/gate.py check` | **以最近一次 check 末行为准**（项数会随判据增加而变）：第 9 轮复核期实跑 **21 PASS / 0 WARN / 0 FAIL**（留档 `doc/05` R-007/R-009；v0.3 处置期的 18 PASS 已被取代） |
...
| 接口链无断点 | 逐信号溯源 | Q3（CSR 写数据）/Q14（预测反馈 4 域）已闭合；**N4/N5/N6（`ftq_id`/`req_id` 键）与 N9/N10（`rt_t` 数据来源、`old_val` 载体）第 8 轮闭合**；**H-01（`disp_x_t.rd_paddr`——写回侧目的号载体）第 10 轮闭合**（`disp_x_t` 322→**328** bit，同步 `spec/00` §4.8 新增 IQ→EU 分发寄存在 1,968 b） |
| 待同步清单 | 逐条跟踪 | `spec/10`（Q27/Q29/Q30 + `csrq` 命中/`wdata_sel` 编码 + `is_sret` 生产点 + `ipend_t` 定义，N11）、`spec/06`（Q33）、`spec/04`（Q16/Q17/Q18 + `HIST_DEPTH`/`ROLLBACK_MAX` 定值并回填 `spec/00` §4.3 + S1/SUS-01 读点口径 + 栈深度推导）、`iss/README.md` §3（Q19① 映射表，C3：**以本篇 §7 为准**）与 §4（签名表扩 `WRITE_MEM`，C2）与异常类”`--final-only`”行（X7）——**iss/README 三处合并登记 `doc/00` ISS-037，同步动作归 R2 角色**、`doc/02`（Q19③/Q29 + “转发后又被冲刷重做”专项）、tb 超时（Q37）、`perf_freelist_stall_cycles` 登记（CLR-03）。**本轮已完成、移出本清单**：`spec/00` §4.8 补 μop/`csrq` 行（X2/BLK-17/CLR-04）、§7.3 `fdirty` 口径（BLK-10/X1）、§9 `PERF_W`/`PERF_EVT_NUM`（X3/C4）、§4.3 `PRF_RD_PORTS=18`（N9）。**引用未建篇目（`spec/02`~`spec/14`，现成稿 2/14）时必须在**首次出现处**标注“该篇待建”**（C7/CLR-02）——**第 10 轮（H-07）补全**：本篇引用的 `spec/02`/`03`/`04`/`05`/`06`/`08`/`09`/`10`/`11`/`13` 十处首次出现已逐一标注（`spec/07`/`12`/`14` 本篇未引用）；后续新增引用同此口径，不逐处重复标注 |

...
| `P5` | `F2` | 取指 | 1 | 指令边界对齐（32B 窗口 → 最多 16 个 16bit 候选槽 / 8 个 32bit 槽）；**含跨窗拼接**（`xw_carry`，ISS-039 定案 A）；写 IBUF | IBUF |
...
| `P9` | `W` | 发射 | 1 | 唤醒（CAM 相联）+ 选择（年龄 + 资源）→ 送执行单元；**依赖间距 3 拍（ADR-4 索引，2026-09-23）**：本行与 P15 的组合＝"唤醒同拍写回 → 选择/发射次拍 → 执行再下拍"＝**3 拍**；**回退条件**见 `doc/00` ISS-012 决策列（T1/T2 命中即升版翻选 2 拍，不动位宽/端口；`spec/13` 成稿时核验） | IQ（清 valid） |
...
（Q14：原 293 且缺预测反馈 4 域，导致前端反馈与 checkpoint 恢复在接口链上断裂，已补齐；N5：再补冲刷键 `ftq_id` 后为 322；**H-01：补 `rd_paddr` 后为 328**，同步 `spec/00` §4.8 **IQ→EU 分发寄存（`disp_x_t`）** 行 1,968 b = `6 × 328`——**I-04 更正注**：第 10 轮原文误写为"级间 μop 寄存（D→R / R→S）"，该行 2,244 b 未动，见 `doc/05` R-010 ④）
...
**指令边界 walk 规则（P5 内部，BLK-08③ 定案；N-04 修正）**：窗口槽号 0..15，槽地址 = `win_base_pc` + 2×槽号；**块首槽 = 块起始 `pc[4:1]`**（`win_base_pc[1:0]` 恒 00，无定位信息，不作定位键）。①从块首槽起逐槽判定：槽半字低 2 bit ≠`11` → 16 bit，下一起始 = 当前+1；= `11` → 32 bit，占当前槽与当前+1 槽，下一起始 = 当前+2（**起始可落任意槽号**，由指令字节对齐决定，无"只落偶数槽"约束）；②32 bit 指令起始槽 =**15** 时跨窗（字节 30..33，缺次窗槽 0 的 2 B）→ **ISS-039 定案（2026-09-23 决策 AI ADR-1，选 A，取代原 E 推迟）**：P5 内增设**尾半字保持寄存器** `xw_carry`：**字段定死＝`{v, lo16, pc[39:0], err, err_code}`，合计 62 bit**（1+16+40+1+4；**A2-D2/B3 回应**：原“上下文”字样已删——本寄存器不承载字段表之外的任何隐含状态，后续如需追加随行信息必须显式列字段并升版）——walk 判定槽 15 为 32bit **起始**时装载之（存 `lo16` 与起始 pc），**本窗 walk 止于槽 15**（不产出该指令、不产出任何"伪起始"）；次窗基址 == 本窗基址+32 时**先产出拼接指令**（`raw32 = {新窗槽 0, carry.lo16}`、`pc = carry.起始 pc`、`is_c = 0`），再自新窗槽 1 续 walk；基址不连续、或冲刷/重定向 ⇒ carry 作废。**"禁跨 32B 边界拼接"据此改为"仅允许经 `xw_carry` 拼接"**；`err` 命中次窗槽 0 时拼接指令带 err 并按 ③ 截断（**拼接不得掩盖 err**）。锚点：规范"指令（16/32bit）对齐于**任意 16bit 边界**"（本机提取件 `riscv_spec_full.txt`）＋ 双参考核同型实现（cv32e40p `aligner` 的 `r_instr_h` 保持尾半字、ibex `fetch_fifo` 的 `rdata_unaligned` 拼接）；几何与吞吐等价性**结论**见 `doc/05` R-017（该件载结论与锚点清单，算式本体归 `spec/03` 细则）。细则（carry 逐拍/冲刷作废、块字节跨度上界 66 B、loop 终止 = **末条指令字节取全**而非 nib 计数完成）归 `spec/03`；**carry 生命周期／顺序／守卫（B3 回应落文）**：① 装载与消费均在 P5 walk 内，P5 对同一取指流**严格按程序序串行处理**（乱序处理窗被禁止；未来若需多窗并行/乱序 walk，本条重开并改多条目或窗标签方案）；② 装载要求 `v=0`，**`v=1` 再装载为协议违例**——置粘滞错误位 **`xw_carry_overrun_sticky`**（候选断言 **A20**：复位后恒真，编号以 §7 汇总为准），禁止静默覆盖；③ 作废条件补全：基址不连续、冲刷、重定向之外，**新增“同基址重取/重放”**——携带期内来源窗或更早窗被重新处理 ⇒ 立即作废；已消费窗被重放 ⇒ 拼接状态随重放机制一并恢复（含 carry 清零与按需重装载），恢复细则归 `spec/03`；④ `v=1` 未消费/未作废期间**全部字段在停顿/反压期间保持稳定**，carry 不改变既有反压路径。**回退点判定标准（B3 回应落文；替换原“亚 32B 取指粒度”单一措辞）**：下列三条任一成立即触发——① 请求粒度（合法取指请求可不以 32B 为最小粒度发出）② 响应粒度（合法响应可只回填窗内部分槽，“16 槽全部可用”不再是响应不变量）③ 可用性粒度（同窗内允许按槽分裂的可用性且无法由 `valid ∧ ¬err + err_pos 截断` 表达）；**触发篇目**：`spec/09`/`spec/10` 定稿逐条对照，`spec/03` 及任何定义取指请求/响应/填充粒度的后续篇目同样对照；成立则按接口升版恢复 `slot16_vld`（或等效按槽可用性位图）。③`err_pos` 命中槽处截断：其后槽不再产出起始，错误经 `err_pos` 展开为 §3.3 按 lane 的 `err`/`err_code`。**err 归属（B-03 补，2026-09-23）**：**其字节跨度含 `err_pos` 槽的指令必须带 err**——含 32bit 指令跨槽情形（起于槽 k 占 k/k+1，`err_pos` 命中 k+1 时该指令带 err，**不得按字面只挂 lane 而让指令静默执行**）；**起于 `err_pos` 之后槽者不产出**（既有句）。**跨窗 err 载体（B3 回应落文，2026-09-23）**：`err` 命中**本窗槽 15** 或**次窗槽 0** 时，拼接指令均带 err——本窗槽 15 的 `err`/`err_code` 由 `xw_carry` **随行锁存**（装载拍按 `err ∧ err_pos==15` 取样）；拼接拍两来源**按位或合成**（任一置位即置 err，`err_code` 取置位来源；双源同时置位取指令首字节所在窗（本窗）侧，若 `spec/10` 异常码表另有定序以 `spec/10` 为准）。锚点：ibex 为跨边界错误专设 `err_plus2`（错误归属随半字跨边界携带）。
...
| A19（可选） | **写只落在 `rob_id` 匹配条目上**：`csrq_wr_fire && csrq_wr_hit \|-> csrq_entry[wr_slot].rob_id == wr_rob_id`（需监视器采样逐条目写使能/槽位）——查"无条件写固定槽"类邻条目污染（A17 查不到的第二种写口错法）。**可选性说明（A1-C3）**：本处"可选"仅指 **SVA 语句落地优先级**，**不豁免该失效模式的验证点覆盖**——是否计入回归面由 `doc/02` 验证点清单决定（"无条件写固定槽"须有验证点） | §3.19 规则 4（C-02 更正：A19 主题＝写口按 `rob_id` 命中落写，属规则 4，原引规则 2 错位） |

| A20（候选） | **`!xw_carry_overrun_sticky`**（复位后恒真）——`v=1` 时再装载（协议违例）须置粘滞位，禁止静默覆盖（B3 回应落文；编号待 §7 汇总确认）。**统计项（非断言）**：`perf_csrq_head_stall_cycles`（队头就绪至发射的等待拍数，软约束；硬上界 `K` 待 `spec/13`） | §3.16/§3.10 |
...
> **两条计数口径的适用面与优先级（C-03 补，2026-09-23）**：①**门禁/评审引用**取"**最新归档件**"——归档件可被第三人复跑，是唯一可回查的权威（本节各行即此口径）；②**改动的 diff 数字对账**取 `doc/05` R-010/R-011 更正注的"**最新一代机判件**"口径，引用时须连带其生成时刻的 HEAD 与基线 commit；③两者相遇（如 r9 的 0 WARN 与 1 WARN 之差）**以归档件为权威**，未落件的实跑只能作"过程记录"引用并**注明未落件**（如 `doc/05` R-009 备注④）。

| 自检项 | 复核手段（可复现命令） | 本轮结果 |
|---|---|---|
| 28 个 struct 合计 = 成员逐项相加，且**标题位宽数与实算一致** | `python script/width_check.py`（v0.3 起含 N16①②③：标题核对、NO-CLAIM 不再计为可核对、转录基准行单列） | **可机器核对 28 / 不符 0 / 未能核实 0 / 基准行 2**（留档：处置期 `doc/05` R-005、复核期 `…/machine_r9_width.txt`；**I-05 处置**：r10 同值件原已存在而未引，本轮起按"引用取最新"补引——第 10 轮复核 `machine_r10_width.txt`、第 11 轮 `machine_r11_width.txt`、**第 12 轮 `machine_r12_width.txt`（最新，A1-C2 补引）**） |
...
| 表格与台账机械核销 | `python script/gate.py check` | **以最近一次 check 末行为准**（项数会随判据增加而变）：最新件 `…/machine_r12_check.txt` 实跑 **22 PASS / 0 WARN / 0 FAIL**（判据数 21→22：新增 `ra-token-defined`）。**历史值按件可回查（B-01 更正）**：`machine_r9_check.txt` 归档件末行＝**21 PASS / 1 WARN / 0 FAIL**（WARN＝rare-chars 低频字 6 个，人工判类），该轮的"0 WARN"出自归档后的末次复跑、**未落件**（出处 `doc/05` R-009 备注④：字频效应使 WARN 归零）；`machine_r10_check.txt`＝21 PASS / 0 WARN / 0 FAIL；v0.3 处置期的 18 PASS 已被取代。口径适用面见本节表头第 2 条 |

...
| v0.1 续1（批 ptw 闭合） | 2026-09-25 | 草稿（待 Requirement Review） | **T-06-5 闭合（回引 `spec/08` §2.1，R-183）** |
...
- **T-06-5**：`ptw_mem_req_t` 的优先级与 L1D 竞争（walk 请求与数据请求仲裁）→ `spec/08` —— **已闭合（→ `spec/08` §2.1，R-183）**：固定优先 walk／不设保留槽／允许同 64B 行合并、禁转发与投机提前。
- **T-06-6**：perf 计数（`MPKI`/`fwd_blocked_replay`）的采样点 → `spec/13` 与 `spec/01` §6 顶层端口表（`perf_*_o` 行）；原引 §3.22 待补，见 ISS-114。
...
| 4 | 结构完整（表列数/粗体/码点） | `python script/flow.py check` 的 `md-integrity`/`codepoints` | 待本轮实跑 |

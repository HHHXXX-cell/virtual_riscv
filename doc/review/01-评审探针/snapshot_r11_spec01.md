...
| `slot16_vld` | 16 | 16 个 16bit 槽各是否**完整落在窗口内**（**待定（I-06）**：窗口 32B 对齐、16×2B 恰铺满全窗，默认模型下该 16 bit **恒 1**（冗余）；是否存在使某槽"不完整"的合法构型（如请求近 64B 行尾被截断）现文未给判据——取值域与不完整构型由 `spec/03`（消费方）/`spec/09` 明确，或按设计选择删该冗余字段，G1 前定稿） |
...
4. **两个写口的"命中失败"语义（H-05）**：`S` 级写 `src_val`、`E` 级写 `old_val` 两写口，均**只在 `rob_id` 命中且该条目仍有效时写入**；未命中（含条目已被规则 1 冲刷截断、或写入到达时该项已提交出队）一律**静默丢弃**，不得改写任何其他条目——若实现成"无条件写固定槽"，迟到写回会污染邻条目；反之，本队列能容忍迟到写回，正是靠"入队序=程序序 + 截断作废 + 写口按 `rob_id` 命中"这三条合力。**待定（ISS-041）**：本条能容忍"写入到达时该项已提交出队"这一情形，**前提是合法写在时序上不可能迟到**——该前提的构造性证明（两写口最迟到达拍数 `k` 的上界，及其与 `csrq` 出队时刻的先后约定）现文未给，且"丢弃"无告警/断言挂钩（§7 无写口断言），由 `spec/13`（时序模型）与 `spec/04` 收口（G1 前必须闭合，回写本行）。**成稿前实现不得违反的约束（R0 2026-09-23 选 C 时锁定）**：**不得把"丢弃"作为合法 CSR 写路径的兜底**——若合法写在无上界保证下被丢弃，`csr_old`（§3.19 `csrq_entry_t.old_val` 行）与 CSRRW 类 `rd_data` 载体缺值，而该 μop 提交仍照常成功（`csr_wr` 由队头 `addr/op/wdata_sel/wdata` 驱动、不依赖 `old_val` 写回），只查提交接口的回归面将**假绿**；闭合前实现至少须让丢弃**可见**（sticky 告警位 + 计数）并在 §7 断言集内挂钩 |
...
| 表格与台账机械核销 | `python script/gate.py check` | **以最近一次 check 末行为准**（项数会随判据增加而变）：最新件 `…/machine_r11_check.txt` 实跑 **21 PASS / 0 WARN / 0 FAIL**（第 9/10 轮复核期同值，分别见 `machine_r9_check.txt`/`machine_r10_check.txt`；v0.3 处置期的 18 PASS 已被取代） |

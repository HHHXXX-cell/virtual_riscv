...
| `P8` | `S` | 分发 | 1 | 就绪初值判定（源已完成或与本拍 `WB` 广播命中；**不读源值**）、分配 ROB / IQ / LQ / SQ、写 `iq_wr_t` | PRF（仅 P9 后读）；ROB/IQ/LQ/SQ（占用计数，分配判据，§4） | ROB、IQ、LQ、SQ |
| `P9` | `W` | 发射 | 1 | 唤醒（CAM 相联）+ 选择（年龄 + 资源）→ 送执行单元；**后读读 PRF（6×2 口）→ `disp_x_t`**；**依赖间距 3 拍（ADR-4 索引，2026-09-23）**：本行与 P15 的组合＝"唤醒同拍写回 → 选择/发射次拍 → 执行再下拍"＝**3 拍**；**回退条件**见 `doc/process/00` ISS-012 决策列（`PT1`/`PT2` 命中即升版翻选 2 拍，不动位宽/端口；`spec/13` 成稿时核验）。**触发口径衔接（D-7）**：§1.1 关键时序结论段的"IPC 达不到 1.8→回炉"是**总体目标线**，**翻选的具体触发以免已量化的 `PT1`（均值 IPC < 1.71）/`PT2`（f>60% ∧ C∈[4,5]）为准**——两者不同时以 `PT1`/`PT2` 为准 | IQ（就绪位/`psrc`/`age`，CAM 与选择）、PRF（后读 6×2 口） | IQ（清 valid） |
...
**A8 补注（D-14 章节片①，2026-09-23）**：“读哪些结构”列为本次补全，读集自“主要动作”各句逐项归位；“—”表示该级无共享结构读（仅消费本级间流水寄存器或由专用通道到达）。

**A9 定义（D-14 决策片，2026-09-23，见 `doc/verify/05` R-049）**：**后读**工程内定义 ＝ **出队（选择）拍读 PRF**；源值唯一载体 ＝ §3.11 `disp_x_t`（`rs1_val/rs2_val` 128 bit）。`P8`/`S` 只做就绪初值判定、**不读源值**。

**F5 改注（D-14 决策片，2026-09-23，见 `doc/verify/05` R-049）**：本表 `P9` 行的**翻选触发判据**代号＝**`PT1`/`PT2`**（旧名 `T1`/`T2`）；同型撞名三族另两族＝批中止判据（`AGENTS.md` 规则 10，改 `AB1`/`AB5`/`AB6`）与 `spec/02` 的 `T-1..T-19`（维持原名），三族自此分域。
...
`raw32`/`pred_target` 在此级之后仍随流携带（trace 与恢复需要）——**B1 澄清：本句指 μop 链**（`D→R→S`：`uop_t`/`rmap_uop_t` 携带 `raw32`）；**ROB 链是另一条**：自 `S` 级起 `iq_wr_t`（§3.8）不含 `raw32`，该字由 `rob_entry_t`（§3.7）一次存储，trace 按 §3.21④ 取 ROB；两条链各有所指、非矛盾。`pred_target` 两链皆需：`iq_wr_t`/`disp_x_t` 均携带（§3.8/§3.11）。
...
**存储体豁免声明与端口方向/速率（A2）**：本节定义的是 ROB **存储阵列**（128 项）的位域，**豁免级间接口的“方向＋条数/拍”标注惯例**；其访问端口方向/速率：写 = `S` 级分配（≤4 条/拍）/ `WB` 置 `finish`（≤6 条/拍）/ `CT` 提交状态更替（≤4 条/拍，§1.3）/ 提交边界中断注入（§5.1）；读 = `CT` 头序访问（≤4 条/拍）与监视器扫描（§7 A6/A13/A16，内部信号口径见 §7 H-06 段）。

### 3.8 `iq_wr_t` — `S → IQ×3`（4 条/拍，带组选）

成员 = 执行与恢复所需全部域（**不含 `raw32`**，避免与 ROB 重复存储——**B1 澄清：本条指 μop 链载荷；`raw32` 归 ROB 链一次存储（§3.7），分工见 §3.5**；Q9：逐成员定宽，**冻结基线禁止“≈”**）：
...
**方向与产生/消费（A3）**：产生级 = `P15`（`WB`），由 `x_complete_t`/`ls_complete_t` 结果组装（§3.20）；方向 = `P15`/`WB` → PRF / ROB / IQ（写广播，≤6 条/拍 = `WAKE_BCAST_PORTS`）；消费方清单 = ① PRF 写口（`paddr` 选条目、`data` 为值）② ROB（`rob_id` 置 `finish`）③ IQ 唤醒广播（`paddr` 为 CAM key，6 端口，§3.9）。
...
| `data` | 64 | 结果 |
| `rob_id` | 8 | 置 `finish` |
| `src_kind` | 1 | 0=执行结果 1=CSR 旧值。**产生点定死（N10）**：E 级 `vr1_csr` 读端口投机读出 old——**执行授权门控（N-02 修正）：该 μop 此刻为 `csrq` 队头**（更老 CSR 均已提交出队）⇒ 读出值必为提交序最新（原稿"`csrq` 空"门控与"`R` 级入队"自相矛盾、恒假，BLK-09 同型"条件永假"错）；提交拍产生的是**写**（`csrq` 出队执行），**读**（`rd_data`/`csr_old` 来源）在执行级完成——原"old 在提交拍组合产生"使 `wb_t.src_kind=1` 无承载通路。**检查点定死（H-05）**：门控在 **`W` 级发射授权拍**判定（该拍判定"此刻为 `csrq` 队头"即授权发射）**〔A2-C3 标记：本句已被 I-01 定案**收窄**——"授权"＝ P9 选择拍的"**选中**"，选中后不存在"进入 `W` 再等"的停放态；勿单读本句理解为"进 W 后等待"〕**，`E` 级沿用授权结果、**不重判**——若在 `E` 级重判，队头可能在 W→E 之间被提交出队而把已授权 μop 半途掐死（执行管道占用语义不同）；门控不成立时该 μop **不消耗 `E` 级拍**。**I-01 定案（ISS-041 选 A，2026-09-23 决策 AI ADR-2，取代原 C 推迟）**：门控**并入 P9 发射选择拍**——不满足即**不选中**该 μop（条目留 IQ，`valid`/`rdy1`/`rdy2` 不变，**每拍重判**；门控是与"源就绪"并列的第二类可发射条件），**取消一切"`W` 级停放"状态**：**无停放槽**（等待只发生在既有 IQ 条目内）、**不阻断**他人发射、等待**出口**为寄存器直比（`rob_id == csrq_head.rob_id`，随更老 CSR 提交单调推进）。**死锁自由论证（构造证明）**：等待者不占 `W`/`E`、不挡他人选择与提交、不持有他人所需资源 ⇒ 一切早于它的 μop 推进与它无关，而其前的 CSR 必先出队 ⇒ 出口有限步可达；CSR 本须串行放行（一次只放队头），故选前过滤与"停放"对 CSR 吞吐等价，差异只在失败路径不占通道。**【公平性前提（B3 回应落文，2026-09-23）】** 上述"有限步可达"以 P9 选择**公平性**为前提：任意已就绪且未被结构性排除的 μop 不得被无限期跳过（**有界跳过／老化即可满足**，不要求固定优先级序）；**`csrq` 队头 μop 一旦就绪，必在有限次选择内发射**——"资源优先且可无限期越过队头"的策略被本前提禁止（可致 CSR 链无界等待，且 A15/A16 对该形态为真空）。检测：统计项 **`perf_csrq_head_stall_cycles`**（队头就绪至其发射的等待拍数，回归监视上界；硬上界 **`K_stall`**（**BLK-1 改名**：与 §3.19 规则 4 的写口在途界 `K` 是两个量，原同用 `K` 致一符两义）待 `spec/04` 定值，定值后升级为断言）。**本区禁止任何"占住 `W` 等条件"的实现**（该读法可构造环等死锁：年轻 CSR 占 W 阻断更老 μop ⇒ 更老 CSR 永不提交 ⇒ 门控永假）。§7 **A15/A16** 为其末点与更早证据 |
...
| `priv` | 2 | 当前特权级 |
| `rw` | 2 | 访问权限检查。**值域（D-14 决策片 A7 定案，见 `doc/verify/05` R-049）：00=R / 01=W / 10=X / 11=保留**——保留码**发出即违例**、接收端按 access-fault 兜底；`kind=11`（PTW）时本域携“触发 walk 的原始访问类型”（PTW 类口径细化归 `spec/08`（该篇待建）） |
| `ftq_id` | 5 | 触发本翻译的 FTQ 项（N4：PTW fill 写 TLB 前须按 A5 与冲刷集比对——原断言引用的键在任何 struct 都不存在；仅 IF/PTW 类请求有效） |
| `lqid` | 6 | Q23 定宽 |
| `robid` | 8 | |

合计（逐成员相加）= 1+40+2+2+2+5+6+8 = **66 bit**（原写 69 为漏项；N4：新增 `ftq_id` 后为 67；**A7：删 `fetch`（≡`kind==2` 冗余）后为 66**，见 `doc/verify/05` R-049）
...
**方向与生产/消费（A4）**：方向 = `MSHR/L2 → L1D/L1I`（唤醒）；产生者 = MSHR（位于 `vr1_l2` 侧；本地副本表归 `vr1_l2`）；消费者 = 对应 L1（L1D/L1I）的缺失等待路径——`ok=1` ⇒ 按 `idx` 命中本地副本、以 `fill_t`（§3.15）完成其名下等待的请求；`ok=0` ⇒ 该 `idx` 名下等待请求按 `code` 判失败、**不得滞留等待**（失败细分→异常的映射归 `spec/09`（该篇待建）/`spec/10`（该篇待建））。
...
（B5 修正：原 `entry_vld2[7:0]` + `entry_pc[7:0]` 方案既无基址载体、又覆盖不了 16 条全 RVC 展开、且未定义 32bit 起始于奇数槽的边界；改用**基址 + 槽标志**表达。**BLK-08/N12：`slot16_start` 从本接口删除**——指令起始判定要读半字内容（低 2 bit ≠`11` 为 16 bit），属译码动作，L1I 不做；起始位图由 P5（`vr1_ifetch` 内部）按下述规则 walk 产出。**I-06 定案（ISS-045，2026-09-23 决策 AI ADR-3）：删 `slot16_vld` 16 bit，338→322**——窗恒按 32B 对齐（P4：64B 行内按 VA[5] 选窗）、槽地址 = 基址+2×槽号 ⇒ 任何合法构型下 16 槽恒含于窗、该字段恒 1（冗余，无"不完整构型"可定义）；"半字可用性"由 **`valid ∧ ¬err` ⇒ 全 16 槽可用 + ③`err_pos` 截断**替代，无信息缺口。**回退点**：若 `spec/09`/`spec/10` 定稿引入亚 32B 取指粒度（PMP/PMA 按槽分裂权限或部分填充响应），按接口升版恢复该字段。）

**指令边界 walk 规则（P5 内部，BLK-08③ 定案；N-04 修正）**：窗口槽号 0..15，槽地址 = `win_base_pc` + 2×槽号；**块首槽 = 块起始 `pc[4:1]`**（`win_base_pc[1:0]` 恒 00，无定位信息，不作定位键）。①从块首槽起逐槽判定：槽半字低 2 bit ≠`11` → 16 bit，下一起始 = 当前+1；= `11` → 32 bit，占当前槽与当前+1 槽，下一起始 = 当前+2（**起始可落任意槽号**，由指令字节对齐决定，无"只落偶数槽"约束）；②32 bit 指令起始槽 =**15** 时跨窗（字节 30..33，缺次窗槽 0 的 2 B）→ **ISS-039 定案（2026-09-23 决策 AI ADR-1，选 A，取代原 E 推迟）**：P5 内增设**尾半字保持寄存器** `xw_carry`：**字段定死＝`{v, lo16, pc[39:0], err, err_code}`，合计 62 bit**（1+16+40+1+4；**A2-D2/B3 回应**：原“上下文”字样已删——本寄存器不承载字段表之外的任何隐含状态，后续如需追加随行信息必须显式列字段并升版）——walk 判定槽 15 为 32bit **起始**时装载之（存 `lo16` 与起始 pc），**本窗 walk 止于槽 15**（不产出该指令、不产出任何"伪起始"）；次窗基址 == 本窗基址+32 时**先产出拼接指令**（`raw32 = {新窗槽 0, carry.lo16}`、`pc = carry.起始 pc`、`is_c = 0`），再自新窗槽 1 续 walk；基址不连续、或冲刷/重定向 ⇒ carry 作废。**"禁跨 32B 边界拼接"据此改为"仅允许经 `xw_carry` 拼接"**；`err` 命中次窗槽 0 时拼接指令带 err 并按 ③ 截断（**拼接不得掩盖 err**）。锚点：规范"指令（16/32bit）对齐于**任意 16bit 边界**"（本机提取件 `riscv_spec_full.txt`）＋ 双参考核同型实现（cv32e40p `aligner` 的 `r_instr_h` 保持尾半字、ibex `fetch_fifo` 的 `rdata_unaligned` 拼接）；几何与吞吐等价性**结论**见 `doc/verify/05` R-017（该件载结论与锚点清单，算式本体归 `spec/03` 细则）。细则（carry 逐拍/冲刷作废、块字节跨度上界 66 B、loop 终止 = **末条指令字节取全**而非 nib 计数完成）归 `spec/03`；**carry 生命周期／顺序／守卫（B3 回应落文）**：① 装载与消费均在 P5 walk 内，P5 对同一取指流**严格按程序序串行处理**（乱序处理窗被禁止；未来若需多窗并行/乱序 walk，本条重开并改多条目或窗标签方案）；② 装载要求 `v=0`，**`v=1` 再装载为协议违例**——置粘滞错误位 **`xw_carry_overrun_sticky`**（候选断言 **A20**：复位后恒真，编号口径以 §7 A20 行为准（待 `spec/11` 成稿汇总定稿；L§1 澄清）），禁止静默覆盖；③ 作废条件补全：基址不连续、冲刷、重定向之外，**新增“同基址重取/重放”**——携带期内来源窗或更早窗被重新处理 ⇒ 立即作废；已消费窗被重放 ⇒ 拼接状态随重放机制一并恢复（含 carry 清零与按需重装载），恢复细则归 `spec/03`；④ `v=1` 未消费/未作废期间**全部字段在停顿/反压期间保持稳定**，carry 不改变既有反压路径。**回退点判定标准（B3 回应落文；替换原“亚 32B 取指粒度”单一措辞）**：下列三条任一成立即触发——① 请求粒度（合法取指请求可不以 32B 为最小粒度发出）② 响应粒度（合法响应可只回填窗内部分槽，“16 槽全部可用”不再是响应不变量）③ 可用性粒度（同窗内允许按槽分裂的可用性且无法由 `valid ∧ ¬err + err_pos 截断` 表达）；**触发篇目**：`spec/09`/`spec/10` 定稿逐条对照，`spec/03` 及任何定义取指请求/响应/填充粒度的后续篇目同样对照；成立则按接口升版恢复 `slot16_vld`（或等效按槽可用性位图）（**L§1 归一口径**：本节两处回退点措辞为"细化"关系，**以三条件判定标准为准**）。③`err_pos` 命中槽处截断：其后槽不再产出起始，错误经 `err_pos` 展开为 §3.3 按 lane 的 `err`/`err_code`。**err 归属（B-03 补，2026-09-23）**：**其字节跨度含 `err_pos` 槽的指令必须带 err**——含 32bit 指令跨槽情形（起于槽 k 占 k/k+1，`err_pos` 命中 k+1 时该指令带 err，**不得按字面只挂 lane 而让指令静默执行**）；**起于 `err_pos` 之后槽者不产出**（既有句）。**跨窗 err 载体（B3 回应落文，2026-09-23）**：`err` 命中**本窗槽 15** 或**次窗槽 0** 时，拼接指令均带 err——本窗槽 15 的 `err`/`err_code` 由 `xw_carry` **随行锁存**（装载拍按 `err ∧ err_pos==15` 取样）；拼接拍两来源**按位或合成**（任一置位即置 err，`err_code` 取置位来源；双源同时置位取指令首字节所在窗（本窗）侧，若 `spec/10` 异常码表另有定序以 `spec/10` 为准）。锚点：ibex 为跨边界错误专设 `err_plus2`（错误归属随半字跨边界携带）。
...
| `op` | 2 | |
| `wdata_sel` | 2 | |
| `src_val` | 64 | `rs1_val`（RS/RC/RW）；立即数型由 `wdata_sel` 指定取 `imm`。**回填写口定案（N-05）**：寄存器型在 `W` 后读拍按 `rob_id` 命中写入（`rs1_val` 在后读拍（`W`/`P9`）才读出，入队拍尚不存在；A9 定案见 §1.1），I 型在 `R` 级入队时即写定——队列因此需按 `rob_id` 的命中写口（`S` 写 `src_val`、`E` 写 `old_val`，同一机制） |
...
**方向与生产/消费（A6）**：产生者 = 各执行单元（单元清单与分组归 `spec/05`（该篇待建））；方向 = `EU → P15`/`WB`（≤6 条/拍，与 §1.3“写回 6 结果”同宽）；消费者 = `WB` 级——据本结构组装 `wb_t` 的三消费面（PRF 写口、置 `finish`、IQ 唤醒广播，§3.10）。
...
**方向与生产/消费（A6）**：产生者 = LSU（访存完成路径：load 数据已回 / fault 判定闭环时）；方向 = `LSU → LQ/ROB`（完成回报，≤2 条/拍 = `MEM≤2`，§1.3）；消费者 = LQ（按 `lqid` 标条目完成态）与 ROB（置 `mem_done`，§3.7；load 可提交判据 `lq_dealloc` 见 §3.17）；`fault`/`code` 汇入提交侧异常路径（§3.18）。
...
|---|---|---|---|---|---|
| BP1 | `rob_near_full`（占用 ≥ `ROB_NEAR_FULL`，即 `ROB_ENTRIES - free >= ROB_NEAR_FULL`；Q31 统一引用绝对水位，BLK-09：原式方向写反——`ROB_ENTRIES - free < ROB_NEAR_FULL` 等于“空闲不足 120 才触发”，几乎全程触发） | `S` 级停止分发 | 组合门控，不冲刷 | ROB 有 ≥ `DECODE_WIDTH` 空闲 | 不变式 **`INV-4`（旧编号 `I1`/`I2`，对照表见 `spec/04`）：`ROB 可提交条目数 > 0`（至少一个槽能被撤销）**，直接决定不死锁 |
...
- **无 checkpoint（逐条回滚）**：**5 + ceil(待撤销映射数 / `ROLLBACK_RATE`)** 拍；`ROLLBACK_RATE=8`。**待撤销数上限取参数 `ROLLBACK_MAX`（**已于 2026-09-23 定值＝33**（ADR-S2），`spec/04` 只做复核与联动公式）——不得写死 128**（B7）；新机制下上界推导收敛为 `ROB_ENTRIES`（栈只存已过 `R` 级的写 RAT μop，`R` 级在途组由本级寄存器恢复，原"+`R` 级在途组"不再需要）。原稿宣称"冲刷 5 拍完成"对无 checkpoint 分支不成立。
...
**快照字段的位宽与载体（H-04 补齐）**：`FL_ck[5:0]`——与 free-list 分配指针**同宽**（`PRF_INT_ENTRIES=64`（F4 统一权威名） ⇒ 6 bit，`spec/00` §4.3）；`SP_ck`——宽度 = `log2(HIST_DEPTH)`，`HIST_DEPTH` **已定值＝33**（ADR-S2；`SP_ck` 因此回落为 **6 bit**）（历史注：原为 `spec/00` §4.3 占位"待定"），**故 `SP_ck` 位宽随该参数定值后代入，**HIST_DEPTH=33**（F8 补：ADR-S2 已定值，原"本成员标记待定"作废）**；快照存储量 = `CHECKPOINT_NUM × (RAT 整表 + FL_ck + SP_ck) = 4 × (192 + 6 + 待定)` bit，归 `spec/04` 容量表与 `spec/00` §4.8（X4/ISS-003：`HIST_DEPTH` 未定即污染冻结面）。
...
| A17 | **未命中丢弃只在冲刷截断覆盖内合法**（I-02 的假绿源变硬红）：`(csrq_wr_fire && !csrq_wr_hit) \|-> (flush_apply && !older(wr_rob_id, flush_br_rob_id)) \|\| ($past(flush_apply,1) && !older(wr_rob_id, $past(flush_br_rob_id,1))) \|\| ($past(flush_apply,2) && !older(wr_rob_id, $past(flush_br_rob_id,2)))`（两写口各挂一份；窗口 `K` 随 `spec/04` 定值调整——B3b-2 改判）——不命中即**合法写被静默丢** | §3.19 规则 4 |
...
| A19（可选） | **写只落在 `rob_id` 匹配条目上**：`csrq_wr_fire && csrq_wr_hit \|-> csrq_entry[wr_slot].rob_id == wr_rob_id`（需监视器采样逐条目写使能/槽位）——查"无条件写固定槽"类邻条目污染（A17 查不到的第二种写口错法）。**可选性说明（A1-C3）**：本处"可选"仅指 **SVA 语句落地优先级**，**不豁免该失效模式的验证点覆盖**——是否计入回归面由 `doc/verify/02` 验证点清单决定（"无条件写固定槽"须有验证点） | §3.19 规则 4（C-02 更正：A19 主题＝写口按 `rob_id` 命中落写，属规则 4，原引规则 2 错位） |

| A20（候选） | **`!xw_carry_overrun_sticky`**（复位后恒真）——`v=1` 时再装载（协议违例）须置粘滞位，禁止静默覆盖（B3 回应落文；编号待 `spec/11` 成稿落 SVA 时汇总定稿（**取代本行后句"§7 汇总确认"表述**，SUS-1 澄清））。**统计项（非断言）**：`perf_csrq_head_stall_cycles`（队头就绪至发射的等待拍数，软约束；硬上界 **`K_stall`** **待 `spec/04` 定值**——D-8/BLK-1：与 §3.19 规则 4 的 `K` 为**不同参数**、已改名消歧；`spec/13` 仅引用不改值）。**A20 编号状态（CL-1 裁定）**：A20 明示为**候选、本轮不计入接受面**——编号待 **`spec/11` 成稿落 SVA 时汇总定稿**（满足 ISS-044 条件②的"明示候选不编号"分支） | §3.16/§3.10 |
...
> **两条计数口径的适用面与优先级（C-03 补，2026-09-23）**：①**门禁/评审引用**取"**最新归档件**"——归档件可被第三人复跑，是唯一可回查的权威（本节各行即此口径）；②**改动的 diff 数字对账**取 `doc/verify/05` R-010/R-011 更正注的"**最新一代机判件**"口径，引用时须连带其生成时刻的 HEAD 与基线 commit；③两者相遇（如 r9 的 0 WARN 与 1 WARN 之差）**以归档件为权威**，未落件的实跑只能作"过程记录"引用并**注明未落件**（如 `doc/verify/05` R-009 备注④）。

| 自检项 | 复核手段（可复现命令） | 本轮结果 |
|---|---|---|
| 28 个 struct 合计 = 成员逐项相加，且**标题位宽数与实算一致** | `python script/width_check.py`（v0.3 起含 N16①②③：标题核对、NO-CLAIM 不再计为可核对、转录基准行单列） | **可机器核对 28 / 不符 0 / 未能核实 0 / 基准行 2**（留档：处置期 `doc/verify/05` R-005、复核期 `…/machine_r9_width.txt`；**I-05 处置**：r10 同值件原已存在而未引，本轮起按"引用取最新"补引——第 10 轮复核 `machine_r10_width.txt`、第 11 轮 `machine_r11_width.txt`、**第 12 轮 `machine_r12_width.txt`（最新，A1-C2 补引）**） |
...
| 待同步清单 | 逐条跟踪 | **（D-10 补登记，2026-09-23）断言接口名随 `spec/11` 成稿核对**（ISS-044 条件③）：§7 A15~A20 引用的监视器信号名与采样点，须在 `spec/11` 成稿时逐条绑定核对（H-06 段为其口径载体）。 `spec/10`（Q27/Q29/Q30 + `csrq` 命中/`wdata_sel` 编码 + `is_sret` 生产点 + `ipend_t` 定义，N11）、`spec/06`（Q33）、`spec/04`（Q16/Q17/Q18 + `HIST_DEPTH`/`ROLLBACK_MAX` **已由 ADR-S2 定值＝33 并回填** `spec/00` §4.3（本项余"复核与随 PRF 联动公式"） + S1/SUS-01 读点口径 + 栈深度推导）、`iss/README.md` §3（Q19① 映射表，C3：**以本篇 §7 为准**）与 §4（签名表扩 `WRITE_MEM`，C2）与异常类”`--final-only`”行（X7）——**iss/README 三处合并登记 `doc/process/00` ISS-037，同步动作归 R2 角色**、`doc/verify/02`（Q19③/Q29 + “转发后又被冲刷重做”专项）、tb 超时（Q37）、`perf_freelist_stall_cycles` 登记（CLR-03）。**本轮已完成、移出本清单**：`spec/00` §4.8 补 μop/`csrq` 行（X2/BLK-17/CLR-04）、§7.3 `fdirty` 口径（BLK-10/X1）、§9 `PERF_W`/`PERF_EVT_NUM`（X3/C4）、§4.3 `PRF_RD_PORTS=18`（N9）。**引用未建篇目（`spec/02`~`spec/14`，现成稿 2/14）时必须在**首次出现处**标注“该篇待建”**（C7/CLR-02）——**第 10 轮（H-07）补全**：本篇引用的 `spec/02`/`03`/`04`/`05`/`06`/`08`/`09`/`10`/`11`/`13` 十处首次出现已逐一标注（`spec/07`/`12`/`14` 本篇未引用）；后续新增引用同此口径，不逐处重复标注 |

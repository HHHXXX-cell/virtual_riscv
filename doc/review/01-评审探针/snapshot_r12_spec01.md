...
**指令边界 walk 规则（P5 内部，BLK-08③ 定案；N-04 修正）**：窗口槽号 0..15，槽地址 = `win_base_pc` + 2×槽号；**块首槽 = 块起始 `pc[4:1]`**（`win_base_pc[1:0]` 恒 00，无定位信息，不作定位键）。①从块首槽起逐槽判定：槽半字低 2 bit ≠`11` → 16 bit，下一起始 = 当前+1；= `11` → 32 bit，占当前槽与当前+1 槽，下一起始 = 当前+2（**起始可落任意槽号**，由指令字节对齐决定，无"只落偶数槽"约束）；②32 bit 指令起始槽 =**15** 时跨窗（字节 30..33，缺次窗槽 0 的 2 B）→ **ISS-039 定案（2026-09-23 决策 AI ADR-1，选 A，取代原 E 推迟）**：P5 内增设**尾半字保持寄存器**（建议名 `xw_carry`：`{v, lo16, 起始 pc[39:0], 上下文}` ≥57 bit）——walk 判定槽 15 为 32bit **起始**时装载之（存 `lo16` 与起始 pc），**本窗 walk 止于槽 15**（不产出该指令、不产出任何"伪起始"）；次窗基址 == 本窗基址+32 时**先产出拼接指令**（`raw32 = {新窗槽 0, carry.lo16}`、`pc = carry.起始 pc`、`is_c = 0`），再自新窗槽 1 续 walk；基址不连续、或冲刷/重定向 ⇒ carry 作废。**"禁跨 32B 边界拼接"据此改为"仅允许经 `xw_carry` 拼接"**；`err` 命中次窗槽 0 时拼接指令带 err 并按 ③ 截断（**拼接不得掩盖 err**）。锚点：规范"指令（16/32bit）对齐于**任意 16bit 边界**"（本机提取件 `riscv_spec_full.txt`）＋ 双参考核同型实现（cv32e40p `aligner` 的 `r_instr_h` 保持尾半字、ibex `fetch_fifo` 的 `rdata_unaligned` 拼接）；几何与吞吐等价性算式见 `doc/05` R-017。细则（carry 逐拍/冲刷作废、块字节跨度上界 66 B、loop 终止 = **末条指令字节取全**而非 nib 计数完成）归 `spec/03`；③`err_pos` 命中槽处截断：其后槽不再产出起始，错误经 `err_pos` 展开为 §3.3 按 lane 的 `err`/`err_code`。
...
| A17 | **未命中丢弃只在冲刷截断覆盖内合法**（I-02 的假绿源变硬红）：`(csrq_wr_fire && !csrq_wr_hit) \|-> (flush_apply && !older(wr_rob_id, flush_br_rob_id)) \|\| ($past(flush_apply,1) && !older(wr_rob_id, $past(flush_br_rob_id,1))) \|\| ($past(flush_apply,2) && !older(wr_rob_id, $past(flush_br_rob_id,2)))`（两写口各挂一份；窗口 `K` 随 `spec/13` 定值调整）——不命中即**合法写被静默丢** | §3.19 规则 4 |
| A18 | **`!csrq_drop_illegal_sticky`**（复位后恒真）——DUT 侧独立通道，与 A17 互为独立证据（硬件位 vs TB 属性） | §3.19 规则 4 |
| A19（可选） | **写只落在 `rob_id` 匹配条目上**：`csrq_wr_fire && csrq_wr_hit \|-> csrq_entry[wr_slot].rob_id == wr_rob_id`（需监视器采样逐条目写使能/槽位）——查"无条件写固定槽"类邻条目污染（A17 查不到的第二种写口错法） | §3.19 规则 2 |
...
| 自检项 | 复核手段（可复现命令） | 本轮结果 |
|---|---|---|
| 28 个 struct 合计 = 成员逐项相加，且**标题位宽数与实算一致** | `python script/width_check.py`（v0.3 起含 N16①②③：标题核对、NO-CLAIM 不再计为可核对、转录基准行单列） | **可机器核对 28 / 不符 0 / 未能核实 0 / 基准行 2**（留档：处置期 `doc/05` R-005、复核期 `…/machine_r9_width.txt`；**I-05 处置**：r10 同值件原已存在而未引，本轮起按"引用取最新"补引——第 10 轮复核 `machine_r10_width.txt`、**第 11 轮 `machine_r11_width.txt`（最新）**） |

...
`D:\IC验证知识库\AI验证工作流程\AI验证工作流程.md` 的九步流水线；
项目级总框架另参《AI进行IC开发验证工作流程》（IPD × 四角色分工）。
...
**P0 环境骨架完成 + P1 规格书 / P2 ISS 并行推进中**（见 [doc/verify/04-验证进度.md](doc/verify/04-验证进度.md)）：
...
| 目录 | 内容 | 可写性 |
|---|---|---|
| `AGENTS.md` | AI 项目级上下文：目录约定/工具链命令/签核标准/23 条工作规则 | 每阶段更新 |
| `doc/`（顶层） | 治理类：`项目开发流程.md` / `AI角色与职责.md` / `环境搭建.md` / `交接说明.md` | AI 可写 |
| `doc/spec/` | ★ 当前主线交付：VR1 微架构规格书 00~14（设计承诺） | AI 可写 |
| `doc/verify/` | 九步产出物 `01~10`（现有 03-验证计划 / 04-验证进度 / 05-回归记录，含台账镜像与驱动计划） | AI 必写 |
| `doc/review/` | 评审记录与评审探针（按篇目编号：`01-评审记录.md` / `01-评审探针/`） | 评审 AI 只写自己的记录 |
| `doc/design/` | 设计侧非规格产物（ADR 汇总 / 接口冻结记录 / 模块实现说明；当前预留） | AI 可写 |
| `doc/项目开发流程.md` | **项目级流程定义**：阶段/门禁（TR↔Review↔基线）、验证策略五问、场景 12 维与验证点 34 法的 VR1 映射、七大收敛指标、四方归因链 | 人批准后方可变更 |
| `doc/process/00-问题记录.md` | 问题台账（红线 R4），发现即登记 | AI 必写 |

...
1. 读 `doc/spec/` 对应篇与 `doc/02` 验证点表，明确本项要验什么、检查手段是什么
...
- 每个新检查点配一次反证实验（能报错才叫检查器），并把注入实验输出记入 `doc/05`
- 认定 ISS 自身有错时：先在 `doc/00` 立"验证代码"条目并注明规范条款出处，再改，再同种子复验
- 随机失败先用同一种子复现，修复后先跑出错种子再进回归；种子全部记 `doc/05`
- 交付前跑 `python script/gate.py check`；比对器与归一化脚本的口径改动要显式声明

**禁止做：**

- 禁止改 `rtl/`（红线 R1）；禁止改 `doc/spec/`（歧义退回 designer/R1）
- 禁止为让结果通过而放宽比对、降级 ERROR、注释断言、删 bins、改采样条件、加无意义 cover（红线 R6/R7）；`compare_final_value_only` 之类放宽只能按 testlist 白名单启用且逐次记 `doc/00`

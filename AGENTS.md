...
- **验证阶段**（AI 每次会话结束前更新此项）：
  **Step 1 理解规格**（进行中——`doc/spec/` 已成 2/14 篇；**`spec/01` v0.3 返工链已走完 D-14 全量复核**：
  章①~⑤＋纵①~③＋六轮复核（A1~A6，评审记录**附 M~附 S**；决策批 ADR-B1~B9/D3/D4/D5、落文批 L1~L9），
  **C2 两要件按字面成立、判定材料齐备、呈 R0 裁决**（`doc/verify/05` R-085；Q-034 未自置 done）；
  台账 61 条＝3 待决策／29 处理中／29 已解决）

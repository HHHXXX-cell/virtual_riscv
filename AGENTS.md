...
> AI 进入项目会话先读本文件，再按 §5 规则与《AI验证工作流》九步流水线工作。
...
- **规格文件**：`doc/spec/00-总体规格.md` ~ `14-接口冻结记录.md`（14 篇，版本 v0.1 草稿，逐篇升版）
...
- **验证阶段**（AI 每次会话结束前更新此项）：
  **Step 1 理解规格**（进行中——spec 为自产文档，`doc/01` 以 `doc/spec/` 已定稿篇目为输入滚动生成；
  `doc/spec/` 已成 2/14 篇）
  **当前工作已暂停**：2026-09-22 交接点，工程拟换到另一台电脑继续，接手说明见
  `doc/交接说明.md`（含新机搬迁校验命令与接续提示词）
- **基线状态**：**未基线**
  - 基线触发点预告（满足其一即可由人宣布）：① Review G1 接口冻结签署；② UVM 平台 smoke 用例通过。
    二者取先。基线后 `rtl/` 转只读、平台改动走 R5。
- **工作流规则**：项目级流程定义（阶段/门禁/度量/管理）见 **`doc/项目开发流程.md`**；
  AI 角色分工与独立性机制（R0~R7、读写边界、对抗评审）见 **`doc/AI角色与职责.md`**；
  执行层九步流水线见 `D:\IC验证知识库\AI验证工作流\AI验证工作流.md`（冲突时以项目流程文件为准）

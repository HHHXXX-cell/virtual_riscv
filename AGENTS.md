...
> **红线记号（RA 系列）说明**（2026-09-23 加，登记 `doc/00` ISS-048）：**RA1~RA4** 出自知识库
> 《AI进行IC开发验证工作流程》§8——RA1 自治决策不进入关键节点／RA2 升级须走完 L1+L2 并留痕／
> RA3 默认策略只许用于非破坏可回退事项／RA4 审计发现不得删除。**RA5 为本项目口径**（知识库
> **未定义**该编号）：「**评审未收敛不得默认通过、不得宣布关闭或进入下一阶段**」——依据是人选定的
> 自治边界"**门禁前停**（不用 24h 默认策略）"（见 `doc/00` ISS-033）；**项目内一切 RA5 引用一律按此定义**。
> 知识库只读：若日后需在库内补定义，须人明确指令＋双确认＋留痕。

...
- **验证阶段**（AI 每次会话结束前更新此项）：
  **Step 1 理解规格**（进行中——`doc/spec/` 已成 2/14 篇；**`spec/01` v0.3 返工链＝D-14 全量复核**：
  章①~⑤＋纵①~③＋十轮复核（A1~A9，评审记录**附 M~附 V**；决策批 ADR-B1~B9/D3/D4/D5、落文批 L1~L10），
  **R0 裁决走最严格口径：A7 复判 C2 未成立 → 处置（R8 修复批＋L10）→ A8/A9 收口确认：全链条项闭环、C2 两要件按字面成立，待 R0 重判**
  （`doc/verify/05` R-086~R-089；Q-034 未自置 done）；台账 62 条＝3 待决策／29 处理中／30 已解决）
